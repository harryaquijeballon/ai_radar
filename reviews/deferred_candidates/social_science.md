...
- surfaced_by: watchlist:nber-working-papers
- action_needed: confirm exact NBER issue date via dated listing/metadata; archive if in-window.; 09-06: still month-level only, unresolved.
- status: pending
...
- action_needed: confirm exact NBER issue date via dated listing/metadata; archive cross-domain if in-window.; 09-06: still month-level only, unresolved.
- status: pending
